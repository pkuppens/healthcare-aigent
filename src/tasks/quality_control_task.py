...
class QualityControlTask(Task):
    ...
    async def execute(self, text: str, llm: any, db: any, audit_logger: any) -> dict[str, Any]:
        ...
        Args:
            ...
            audit_logger: The logger instance to use for logging audit events.
        ...
        qc_results = await run_llm_json(llm, prompt, REQUIRED_KEYS, action="quality control")

        try:
            await audit_logger.log_audit_event({"event_type": "quality_control", "text_id": hash(text), "results": qc_results})
        except Exception as e:
            logger.error(f"Failed to perform quality control: {e}")
            raise RuntimeError(f"An error occurred during quality control: {e}") from e
