...
async def test_web_search_tool():
    ...
    assert f"Mock search results for: {search_query}" in result, \
        f"Expected mock search results in output, but got: {result}"
...
async def test_patient_language_tool():
    ...
    assert "language_proficiency" in result, \
        f"Missing 'language_proficiency' key in result: {result}"
    assert "medical_literacy" in result, \
        f"Missing 'medical_literacy' key in result: {result}"
    assert "suggested_communication_level" in result, \
        f"Missing 'suggested_communication_level' key in result: {result}"
